...
class MakeCloud():
   ...
   def wc(self, file):
        ...
        value_to_remove = ['기금', '위원회', '정부', '국가', '사항', '계획', '지방자치단체', '것임', '국민', '중립', '장관', '경우']
   ...
   def wc2(self, file):
       ...
       # 색깔 설정 함수
       def color_func(word, font_size, position, orientation, random_state=None, **kwargs):  # color_func의 기본 파라미터들
              return ("hsl({:d},{:d}%, {:d}%)".format(np.random.randint(220, 240), np.random.randint(80, 100),
   ...
   def wc3(self, file):
       ...
       # 색깔 설정 함수
       def color_func(word, font_size, position, orientation, random_state=None, **kwargs):  # color_func의 기본 파라미터들
           return ("hsl({:d},{:d}%, {:d}%)".format(np.random.randint(0, 15), np.random.randint(80, 100),
                                                   np.random.randint(10, 80)))
